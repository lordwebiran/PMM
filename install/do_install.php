...
if (isset($_POST)) {
    ...
    if (!($host && $dbuser && $dbname && $first_name && $last_name && $email && $login_password)) {
        echo json_encode(array("success" => false, "message" => "لطفا تمام فیلدها را وارد کنید."));
        exit();
    }
    ...
    if (filter_var($email, FILTER_VALIDATE_EMAIL) === false) {
        echo json_encode(array("success" => false, "message" => "لطفا یک ایمیل معتبر وارد کنید"));
        exit();
    }
    ...
    if (!is_file('database.sql')) {
        echo json_encode(array("success" => false, "message" => "فایل database.sql در پوشه نصب پیدا نشد!"));
        exit();
    }
    ...
    if (!$is_installed) {
        echo json_encode(array("success" => false, "message" => "به نظر می رسد این برنامه قبلاً نصب شده است! نمی توانید دوباره آن را نصب کنید."));
        exit();
    }
    ...
    echo json_encode(array("success" => true, "message" => "نصب با موفقیت انجام شد."));
    exit();
}